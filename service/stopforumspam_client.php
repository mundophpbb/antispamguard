...
class stopforumspam_client
{
	...
	public function check_many(array $checks)
	{
		...
		if ($response === false)
		{
			...
			foreach ($missing as $type => $value)
			{
				$this->sfs_cache->set_error($type, $value, 'request_failed');
		...
		if (!is_array($data) || (isset($data['success']) && empty($data['success'])))
		{
			...
			foreach ($missing as $type => $value)
			{
				$this->sfs_cache->set_error($type, $value, 'invalid_response');
		...
		foreach ($missing as $type => $value)
		{
			...
			{
				$this->sfs_cache->set_error($type, $value, 'missing_result');
	...
	protected function get_user_agent()
	{
		...
		if (is_readable($composer_file))
		{
			...
			{
				$clean_version = preg_replace('/[^0-9A-Za-z.+-]/', '', (string) $composer['version']);
				$version = $clean_version !== '' ? $clean_version : $version;
			}
		}

		$this->user_agent = 'AntiSpamGuard/' . $version;
