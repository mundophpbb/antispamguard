...
class main_module
{
	...
	{
		global $config, $db, $request, $template, $user, $table_prefix, $phpbb_container;
		...
		if ($request->is_set_post('test_sfs_lookup'))
		{
			...
			if (!$sfs_result || !empty($sfs_result['error']))
			{
				$error_status = ($sfs_result && !empty($sfs_result['error_status']))
					? (string) $sfs_result['error_status']
					: 'request_failed';

				$template->assign_vars(array(
					'S_SFS_TEST_DONE' => true,
					'S_SFS_TEST_ERROR' => true,
					'SFS_TEST_TYPE' => $test_type,
					'SFS_TEST_VALUE' => $test_value,
					'SFS_TEST_CACHED' => ($sfs_result && !empty($sfs_result['cached'])) ? $user->lang('YES') : $user->lang('NO'),
					'SFS_TEST_LISTED' => $user->lang('NO'),
					'SFS_TEST_CONFIDENCE' => 0,
					'SFS_TEST_FREQUENCY' => 0,
					'SFS_TEST_RESULT' => $user->lang('ACP_ANTISPAMGUARD_SFS_TEST_ERROR_STATUS', $error_status),
				));
			}
			else
			{
		...
		if ($request->is_set_post('test_sfs_and_log'))
		{
			...
			$decision = $sfs_decision->should_block($test_ip, $test_email, $test_username, 'manual_acp_test', true);
				...
				$template->assign_vars(array(
					...
					'S_SFS_MANUAL_TEST_ERROR' => !empty($decision['had_error']),
		...
		$recent_alerts = $phpbb_container->get('mundophpbb.antispamguard.alerts')->get_recent(20);
		foreach ($recent_alerts as $alert)
		{
			$template->assign_block_vars('antispamguard_alerts', array(
				'CREATED_AT' => $user->format_date((int) $alert['created_at']),
				'SEVERITY' => (string) $alert['severity'],
				'IP' => (string) $alert['ip'],
				'USERNAME' => (string) $alert['username'],
				'MESSAGE' => (string) $alert['message'],
				'S_READ' => !empty($alert['is_read']),
			));
		}
	...
	protected function export_config_inventory_csv(\phpbb\db\driver\driver_interface $db, \phpbb\user $user)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			fputcsv($output, $this->get_settings_helper()->sanitize_csv_row(array(
				$row['config_name'],
				$config_value,
				(int) $row['is_dynamic'],
			)));
